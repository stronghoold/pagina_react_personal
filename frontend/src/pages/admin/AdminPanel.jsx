...
const AdminPanel = ({ dark, onToggleTheme }) => {
  ...
  const activeLabel = ADMIN_TABS.find((t) => t.id === activeTab)?.label || ''
        ...
        <main className="flex-1 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
          ...
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              Gestiona el sistema, analiza las ventas y atiende las solicitudes de los clientes.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900 sm:p-6">
            {activeTab === 'dashboard' && <DashboardPanel token={token} rol="administrador" />}
            {activeTab === 'sales' && <VentasPanel token={token} usuario="administrador" />}
            {activeTab === 'invoices' && <FacturasPanel token={token} usuario="administrador" />}
            {activeTab === 'reports' && <ReportsPanel token={token} />}
            {activeTab === 'pqr' && <PqrPanel token={token} usuario="administrador" />}
